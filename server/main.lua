...
Citizen.CreateThread(function()
	...
	if Config.Multichar or Config.Identity then	-- append these fields to the select query
		query = query..', `firstname`, `lastname`, `dateofbirth`, `sex`'
	end

	if Config.Multichar then -- insert identity data with creation
		MySQL.Async.store("INSERT INTO `users` SET `accounts` = ?, `citizenid` = ?, `queue` = ?, `group` = ?, `firstname` = ?, `lastname` = ?, `dateofbirth` = ?, `sex` = ?, `height` = ?", function(storeId)
			NewPlayer = storeId
		end)
	else
		MySQL.Async.store("INSERT INTO `users` SET `accounts` = ?, `citizenid` = ?, `queue` = ?, `group` = ?, `firstname` = ?, `lastname` = ?, `dateofbirth` = ?, `sex` = ?", function(storeId)
			NewPlayer = storeId
		end)
	end

	MySQL.Async.store("SELECT "..query.." FROM `users` WHERE citizenid = ?", function(storeId)
		LoadPlayer = storeId
	end)
end)
...
function createESXPlayer(identifier, citizenid, playerId, data)
	local accounts = {}
	...
	if not Config.Multichar then
		MySQL.Async.execute(NewPlayer, {
			...
			loadESXPlayer(identifier, citizenid, playerId, true)
		end)
	else
		MySQL.Async.execute(NewPlayer, {
				json.encode(accounts),
...
function loadESXPlayer(identifier, citizenid, playerId, isNew)
	local tasks = {}
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll(LoadPlayer, { citizenid
		}, function(result)
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, citizenid, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.loadout, userData.playerName, userData.coords)
